...
exports.getAddonsByRentalId = async (req, res) => {
    ...
    try {
        const [rows] = await db.query(
            "SELECT * FROM rentaladdons WHERE rental_id = ?",
            [rental_id]
        );
        if (!rows.length)
            return res.status(404).json({ message: "Không tìm thấy addon" });
        res.json(rows);
    } catch (err) {
...
exports.createAddon = async (req, res) => {
    try {
        const { rental_id, addon_type, addon_name, addon_price, quantity } =
            req.body;
        if (
            !rental_id ||
            !addon_type ||
...
exports.updateAddon = async (req, res) => {
    try {
        ...
        const [[row]] = await db.query(
            "SELECT addon_price FROM rentaladdons WHERE addon_id = ?",
            [addon_id]
        );
        if (!row)
            return res.status(404).json({ message: "Addon không tồn tại" });

        const total_price =
            parseFloat(row.addon_price) * parseInt(quantity, 10);
        await db.query(
            "UPDATE rentaladdons SET quantity = ?, total_price = ? WHERE addon_id = ?",
            [quantity, total_price, addon_id]
        );
...
exports.deleteAddon = async (req, res) => {
    try {
        const { addon_id } = req.params;
        const [result] = await db.query(
            "DELETE FROM rentaladdons WHERE addon_id = ?",
            [addon_id]
        );
        if (result.affectedRows === 0) {
            return res
                .status(404)
                .json({ message: "Không tìm thấy addon để xóa" });
        }
